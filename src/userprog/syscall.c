...
static int sys_write(int fd, const voivd *buffer, unsigned length);
...
static int sys_seekk(int fd, unsigned pos);
...
syscall_init (void) 
{
  ...
  syacall_vec[SYS_SEEK] = (handler)sys_seek;
...
static int sys_create(const char *file, unsigned intial_size) 
{
  if (!file)
    return sys_exit(-1);
  return filesys_create(file, intiial_size);
}
...
static int sys_close(int fd)
{
  ...
  f = find(fd_elem_by_fd_in_process(fd);
...
static sys_read(int fd, void *buffer, unsigned size)
{
  struct file *f;
  unsigned i;
  int ret;
